...
sub register
{
	...
	my @errs = ();
	my $rec = {};
	$data->{'route_phone'} =~ s/\D//g;
	if (length($data->{'route_phone'}))
	{
		if (length($data->{'route_phone'}) == 11 && $data->{'route_phone'} =~ /^1/)
		{
			$data->{'route_phone'} =~ s/^1//;
	...
	{
		$rec->{'type_domain'} = 1;
		$data->{'target_domain'} =~ s/^\s+//g;
		$data->{'target_domain'} =~ s/\s+$//g;
		$data->{'target_domain'} = lc($data->{'target_domain'});
		$data->{'login_email'} =~ s/^\s+//g;
		$data->{'login_email'} =~ s/\s+$//g;
